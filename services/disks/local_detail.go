package disks

import (
	"context"
	"fmt"
	"github.com/clo-ru/cloapi-go-client/clo"
	"net/http"
)

const (
	localDetailEndpoint = "/v1/local-disks/%s/detail"
)

type LocalDetailRequest struct {
	clo.Request
	LocalDiskID string
}

func (r *LocalDetailRequest) Make(ctx context.Context, cli *clo.ApiClient) (LocalDetailResponse, error) {
	rawReq, e := r.buildRequest(ctx, cli.Options)
	if e != nil {
		return LocalDetailResponse{}, e
	}
	rawResp, requestError := r.MakeRequest(rawReq, cli)
	if requestError != nil {
		return LocalDetailResponse{}, requestError
	}
	defer rawResp.Body.Close()
	var resp LocalDetailResponse
	if e = resp.FromJsonBody(rawResp.Body); e != nil {
		return LocalDetailResponse{}, e
	}
	return resp, nil
}

func (r *LocalDetailRequest) buildRequest(ctx context.Context, cliOptions map[string]interface{}) (*http.Request, error) {
	authKey, ok := cliOptions["auth_key"].(string)
	if !ok {
		return nil, fmt.Errorf("auth_key client options should be a string, %T got", authKey)
	}
	baseUrl, ok := cliOptions["base_url"].(string)
	if !ok {
		return nil, fmt.Errorf("base_url client options should be a string, %T got", baseUrl)
	}
	baseUrl += fmt.Sprintf(localDetailEndpoint, r.LocalDiskID)
	rawReq, e := http.NewRequestWithContext(
		ctx, http.MethodGet, baseUrl, nil,
	)
	if e != nil {
		return nil, e
	}
	h := http.Header{}
	h.Add("Authorization", fmt.Sprintf("Bearer %s", authKey))
	r.WithHeaders(h)
	return rawReq, nil
}
