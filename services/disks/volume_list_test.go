...
func TestVolumeListRequest_BuildRequest(t *testing.T) {
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodGet, mocks.MockUrl+fmt.Sprintf(volumeListEndpoint, ID), nil,
	)
	expReq.Header = h
...
func TestVolumeListRequest_Make(t *testing.T) {
	...
	var cases = []struct {
		...
		},
		{
			...
			Expected: VolumeListResponse{Results: []VolumeDetail{}},
		},
	}
	for _, c := range cases {
...
func TestVolumeListPaginator_NextPage(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			pg, e := NewVolumeListPaginator(&cli, req, c.PGOptions)
			assert.Nil(t, e)
...
func TestVolumeListPaginator_lastPage(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
	...
	pg, e := NewVolumeListPaginator(&cli, req, PaginatorOptions{
		Limit:  3,
		Offset: 3,
	})
	assert.Nil(t, e)
