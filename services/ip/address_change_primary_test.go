...
func TestAddressPrimaryChangeRequest_BuildRequest(t *testing.T) {
	fID := "id"
	req := AddressPrimaryChangeRequest{
		AddressID: fID,
	}
	h := http.Header{}
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodPost, mocks.MockUrl+fmt.Sprintf(addressPrimaryChangeEndpoint, fID), nil,
	)
	expReq.Header = h
	assert.Equal(t, expReq, rawReq)
}

func TestAddressPrimaryChangeRequest_Make(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			_, e := c.Req.Make(context.Background(), &cli)
			if !c.ShouldFail {
				assert.Nil(t, e)
			} else {
				assert.NotNil(t, e)
...
func TestAddressPrimaryChangeRequest_MakeRetry(t *testing.T) {
	retry := 5
