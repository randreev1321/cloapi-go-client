...
func TestAddressDetachRequest_BuildRequest(t *testing.T) {
	fID := "id"
	req := AddressDetachRequest{
		AddressID: fID,
	}
	h := http.Header{}
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodPost, mocks.MockUrl+fmt.Sprintf(addressDetachEndpoint, fID), nil,
	)
	expReq.Header = h
...
func TestAddressDetachRequest_Make(t *testing.T) {
	...
	var cases = []struct {
		...
		Expected       AddressDetachResponse
	}{
		{
			Name: "Success",
			BodyStringFunc: func() (string, int) {
			...
			Expected: AddressDetachResponse{Result: AddressDetail{
				ID:     "fipid",
				Status: "ACTIVE",
