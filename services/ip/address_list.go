...
const (
	addressListEndpoint = "/v1/projects/%s/addresses"
)
...
func (r *AddressListRequest) Make(ctx context.Context, cli *clo.ApiClient) (AddressListResponse, error) {
	rawReq, e := r.buildRequest(ctx, cli.Options)
	if e != nil {
		return AddressListResponse{}, e
	}
	rawResp, requestError := r.MakeRequest(rawReq, cli)
	if requestError != nil {
		return AddressListResponse{}, requestError
	}
...
func (r *AddressListRequest) buildRequest(ctx context.Context, cliOptions map[string]interface{}) (*http.Request, error) {
	...
	baseUrl += fmt.Sprintf(addressListEndpoint, r.ProjectID)
...
func (r *AddressListRequest) OrderBy(of string) *AddressListRequest {
	r.WithQueryParams(map[string][]string{"order": {of}})
	return r
}

func (r *AddressListRequest) FilterBy(ff FilteringField) *AddressListRequest {
	switch ff.Condition {
...
type AddressListPaginator struct {
	op       PaginatorOptions
...
func (lp *AddressListPaginator) LastPage() bool {
	return lp.lastPage
}

func NewAddressListPaginator(client *clo.ApiClient, params AddressListRequest, op PaginatorOptions) (*AddressListPaginator, error) {
	if op.Limit == 0 {
		return nil, fmt.Errorf("op.Limit should not be 0")
	}
	lp := AddressListPaginator{
		client: client,
		params: params,
		op:     op,
	}
...
func (lp *AddressListPaginator) NextPage(ctx context.Context) (AddressListResponse, error) {
	if lp.LastPage() {
		return AddressListResponse{}, fmt.Errorf("no more pages")
	}
