...
func TestAddressListRequest_BuildRequest(t *testing.T) {
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodGet, mocks.MockUrl+fmt.Sprintf(addressListEndpoint, ID), nil,
	)
	expReq.Header = h
...
func TestAddressListRequest_Make(t *testing.T) {
	...
	var cases = []struct {
	...
	}{
		{
			Name: "Success",
			BodyStringFunc: func() (string, int) {
				return `{"count": 2, "results": [{"id": "first_item_id", "ptr": "host.com", "ddos_protection":true},{"id": "second_item_id", "ptr": "host.com", "ddos_protection":true}]}`,
					http.StatusOK
			},
			Req: AddressListRequest{
		...
		},
		{
			...
			Expected: AddressListResponse{Results: []AddressDetail{}},
		},
	}
	for _, c := range cases {
...
func TestAddressListPaginator_NextPage(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			pg, e := NewAddressListPaginator(&cli, req, c.PGOptions)
			assert.Nil(t, e)
...
func TestAddressListPaginator_lastPage(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
	...
	pg, e := NewAddressListPaginator(&cli, req, PaginatorOptions{
		Limit:  3,
		Offset: 3,
	})
	assert.Nil(t, e)
...
func TestAddressList_Filtering(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
