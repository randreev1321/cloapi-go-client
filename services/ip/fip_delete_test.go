...
func TestFipDeleteRequest_BuildRequest(t *testing.T) {
	ID := "id"
	req := FipDeleteRequest{
		FloatingID: ID,
	}
	h := http.Header{}
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodDelete, mocks.MockUrl+fmt.Sprintf(fipDeleteEndpoint, ID), nil,
	)
	expReq.Header = h
	assert.Equal(t, expReq, rawReq)
}

func TestFipDeleteRequest_Make(t *testing.T) {
...
func TestFipDeleteRequest_MakeRetry(t *testing.T) {
	retry := 5
