...
const (
	fipDetailEndpoint = "/v1/floatingips/%s/detail"
)

type FipDetailRequest struct {
	clo.Request
	FloatingID string
}

func (r *FipDetailRequest) Make(ctx context.Context, cli *clo.ApiClient) (FipDetailResponse, error) {
	rawReq, e := r.buildRequest(ctx, cli.Options)
	if e != nil {
		return FipDetailResponse{}, e
	}
	rawResp, requestError := r.MakeRequest(rawReq, cli)
	if requestError != nil {
		return FipDetailResponse{}, requestError
	}
	defer rawResp.Body.Close()
	var resp FipDetailResponse
...
func (r *FipDetailRequest) buildRequest(ctx context.Context, cliOptions map[string]interface{}) (*http.Request, error) {
	...
	baseUrl += fmt.Sprintf(fipDetailEndpoint, r.FloatingID)
