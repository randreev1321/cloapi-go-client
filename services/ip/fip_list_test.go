...
func TestFipListRequest_BuildRequest(t *testing.T) {
	ID := "id"
	req := FipListRequest{
		ProjectID: ID,
	}
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodGet, mocks.MockUrl+fmt.Sprintf(fipListEndpoint, ID), nil,
	)
	expReq.Header = h
	assert.Equal(t, expReq, rawReq)
}

func TestFipListRequest_Make(t *testing.T) {
	...
	var cases = []struct {
				...
				Results: []FipDetail{
					{
						ID:               "first_item_id",
						Ptr:              "host.com",
						AttachedToServer: AttachedToEntityDetails{ID: "server_id"},
					},
					{
						ID:               "second_item_id",
						Ptr:              "host.com",
		...
		},
		{
			...
			Expected: FipListResponse{Results: []FipDetail{}},
		},
	}
	for _, c := range cases {
...
func TestFipListRequest_MakeRetry(t *testing.T) {
	retry := 5
...
func TestFipListPaginator_NextPage(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
...
func TestFipListPaginator_lastPage(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
...
func TestFipList_Filtering(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
