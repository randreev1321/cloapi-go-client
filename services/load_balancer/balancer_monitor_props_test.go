...
func TestBalancerChangeMonitorRequest_BuildRequest(t *testing.T) {
	b := BalancerChangeMonitorBody{}
	ID := "id"
	req := BalancerChangeMonitorRequest{
		Body:       b,
		BalancerID: ID,
	}
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodPut, mocks.MockUrl+fmt.Sprintf(balancerChangeMonitorEndpoint, ID), bd,
	)
	expReq.Header = h
...
func TestBalancerChangeMonitorRequest_CheckPassedBody(t *testing.T) {
	erCode := http.StatusInternalServerError
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
	...
	req := BalancerChangeMonitorRequest{
		Body: BalancerChangeMonitorBody{Delay: 2, HttpMethod: "get", MaxRetries: 3},
	}
	_ = req.Make(context.Background(), &cli)
	exp := []byte(`{"delay":2,"timeout":0,"max_retries":3,"type":"","url_path":"","http_method":"get","expected_codes":""}`)
	exp = append(exp, '\n')
