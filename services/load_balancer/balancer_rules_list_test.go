...
func TestBalancerRulesListRequest_BuildRequest(t *testing.T) {
	ID := "id"
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodGet, mocks.MockUrl+fmt.Sprintf(balancerRulesListEndpoint, ID), nil,
	)
	expReq.Header = h
...
func TestBalancerRulesListRequest_Make(t *testing.T) {
	...
	var cases = []struct {
		...
		Expected       BalancerRulesListResponse
	}{
		{
			Name: "Success",
			BodyStringFunc: func() (string, int) {
				return `{"count":2,"results":[{"id":"rule1","external_protocol_port":2,"internal_protocol_port":3,"server":{"id":"serv1"}},{"id":"rule2","external_protocol_port":3,"internal_protocol_port":43,"loadbalancer":{"id":"lb1"}}]}`,
					http.StatusOK
			},
			Req: BalancerRulesListRequest{
			...
			Expected: BalancerRulesListResponse{
				Count: 2,
				Results: []BalancerRulesDetailItem{
					{
						ID:                   "rule1",
						ExternalProtocolPort: 2,
						InternalProtocolPort: 3,
						Server: AttachedToEntityDetails{
							ID: "serv1",
						},
					},
					{
						...
						Loadbalancer: AttachedToEntityDetails{
							ID: "lb1",
						},
					},
				},
		...
		},
		{
			...
			Expected: BalancerRulesListResponse{Results: []BalancerRulesDetailItem{}},
		},
	}
	for _, c := range cases {
...
func TestBalancerRulesListPaginator_NextPage(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			pg, e := NewBalancerRulesListPaginator(&cli, req, c.PGOptions)
			assert.Nil(t, e)
...
func TestBalancerRulesListPaginator_lastPage(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
	...
	pg, e := NewBalancerRulesListPaginator(&cli, req, PaginatorOptions{
		Limit:  3,
		Offset: 3,
	})
	assert.Nil(t, e)
...
func TestBalancerRulesList_Filtering(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
