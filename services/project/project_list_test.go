...
func TestProjectListRequest_BuildRequest(t *testing.T) {
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodGet, mocks.MockUrl+projectListEndpoint, nil,
	)
	expReq.Header = h
...
func TestProjectListRequest_Make(t *testing.T) {
	...
	var cases = []struct {
	...
	}{
		{
			Name: "Success",
			BodyStringFunc: func() (string, int) {
				return `{"count": 2,"results": [{"name":"first_item_name","summary":{"floating_ips":3,"servers":2}},{"name":"second_item_name","summary":{"networks":3}}]}`,
					http.StatusOK
			},
			Req: ProjectListRequest{},
			Expected: ProjectListResponse{
		...
		},
		{
			...
			Req:      ProjectListRequest{},
			Expected: ProjectListResponse{Results: []ProjectListItem{}},
		},
	}
	for _, c := range cases {
...
func TestProjectListPaginator_NextPage(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			req := ProjectListRequest{}
			pg, e := NewProjectListPaginator(&cli, req, c.PGOptions)
			assert.Nil(t, e)
...
func TestProjectListPaginator_lastPage(t *testing.T) {
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
		Options: map[string]interface{}{
	...
	pg, e := NewProjectListPaginator(&cli, req, PaginatorOptions{
		Limit:  3,
		Offset: 3,
	})
	assert.Nil(t, e)
