...
func TestServerChangePasswdRequest_BuildRequest(t *testing.T) {
	b := ServerChangePasswdBody{
		Password: "m",
	}
	ID := "id"
	req := ServerChangePasswdRequest{
		Body:     b,
		ServerID: ID,
	}
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodPost, mocks.MockUrl+fmt.Sprintf(serverChangePasswdEndpoint, ID), bd,
	)
	expReq.Header = h
...
func TestServerChangePasswdRequest_CheckPassedBody(t *testing.T) {
	erCode := http.StatusInternalServerError
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
	...
	req := ServerChangePasswdRequest{
		Body: ServerChangePasswdBody{Password: "pwd"},
	}
	_ = req.Make(context.Background(), &cli)
	exp := []byte(`{"password":"pwd"}`)
	exp = append(exp, '\n')
