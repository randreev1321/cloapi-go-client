...
const (
	serverChangePtrEndpoint = "/v1/servers/%s/ptr"
)

type ServerChangePtrRequest struct {
	clo.Request
	ServerID string
	Body     ServerChangePtrBody
}

type ServerChangePtrBody struct {
	Value string `json:"value"`
}

func (r *ServerChangePtrRequest) Make(ctx context.Context, cli *clo.ApiClient) error {
...
func (r *ServerChangePtrRequest) buildRequest(ctx context.Context, cliOptions map[string]interface{}) (*http.Request, error) {
	...
	baseUrl += fmt.Sprintf(serverChangePtrEndpoint, r.ServerID)
	b := new(bytes.Buffer)
