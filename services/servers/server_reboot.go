...
const (
	serverRebootEndpoint = "/v1/servers/%s/reboot"
)

type ServerRebootRequest struct {
	clo.Request
	ServerID string
	Body     ServerRebootBody
}

type ServerRebootBody struct {
	Reboot ServerRebootType `json:"reboot"`
}

type ServerRebootType struct {
	Type string `json:"type"`
}

func (r *ServerRebootRequest) Make(ctx context.Context, cli *clo.ApiClient) error {
...
func (r *ServerRebootRequest) buildRequest(ctx context.Context, cliOptions map[string]interface{}) (*http.Request, error) {
	...
	baseUrl += fmt.Sprintf(serverRebootEndpoint, r.ServerID)
	b := new(bytes.Buffer)
