...
func TestSnapshotCreateRequest_BuildRequest(t *testing.T) {
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodPost, mocks.MockUrl+fmt.Sprintf(snapCreateEndpoint, ID), bd,
	)
	expReq.Header = h
...
func TestSnapshotCreateRequest_Make(t *testing.T) {
	...
	var cases = []struct {
		...
		Expected       SnapshotCreateResponse
	}{
		{
			Name: "Success",
			BodyStringFunc: func() (string, int) {
				return fmt.Sprintf(`{"result":{"id":"snap_id"}}`),
					http.StatusOK
			},
			Req: SnapshotCreateRequest{
				ServerID: "id",
			},
			Expected: SnapshotCreateResponse{Result: ResponseItem{
				ID: "snap_id",
			}},
		},
		{
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			mocks.CheckHeaders(httpCli.Headers)
...
func TestSnapshotCreateRequest_CheckPassedBody(t *testing.T) {
	erCode := http.StatusInternalServerError
	httpCli := mocks.RequestDebugClient{}
	cli := clo.ApiClient{
		HttpClient: &httpCli,
