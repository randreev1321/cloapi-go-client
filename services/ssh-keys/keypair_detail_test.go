...
func TestKeyPairDetailRequest_BuildRequest(t *testing.T) {
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodGet, mocks.MockUrl+fmt.Sprintf(keypairDetailEndpoint, ID), nil,
	)
	expReq.Header = h
...
func TestKeyPairDetailRequest_Make(t *testing.T) {
	...
	var cases = []struct {
	...
	}{
		{
			Name: "Success",
			BodyStringFunc: func() (string, int) {
				return fmt.Sprintf(`{"result":{"name":"kp_name","id":"id","public_key":"pubkey"}}`),
					http.StatusOK
			},
			Req: KeyPairDetailRequest{
				KeypairID: "id",
			},
			Expected: KeyPairDetailResponse{Result: KeyPairResponseItem{
				ID:        "id",
				Name:      "kp_name",
				PublicKey: "pubkey",
			},
