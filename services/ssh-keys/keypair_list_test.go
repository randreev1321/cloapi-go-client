...
func TestKeyPairListRequest_BuildRequest(t *testing.T) {
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodGet, mocks.MockUrl+keypairListEndpoint, nil,
	)
	expReq.Header = h
...
func TestKeyPairListRequest_Make(t *testing.T) {
	...
	var cases = []struct {
		...
		{
			Name:       "WrongPubkeyReturned",
			ShouldFail: true,
			BodyStringFunc: func() (string, int) {
				return `{"count": 2,"results": [{"name":"first_item_name","public_key":"first_pub"},{"name":"second_item_name","id":"second_item_id"}]}`,
					http.StatusOK
			},
			Req: KeyPairListRequest{},
			Expected: KeyPairListResponse{
		...
		},
		{
			...
			Req:      KeyPairListRequest{},
			Expected: KeyPairListResponse{Results: []KeyPairResponseItem{}},
		},
	}
	for _, c := range cases {
...
func TestKeyPairListPaginator_NextPage(t *testing.T) {
	...
	var cases = []struct {
		...
		Name       string
		Expected   string
		PGOptions  KeyPairPaginatorOptions
	}{
		{
			Name: "Success",
			PGOptions: KeyPairPaginatorOptions{
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			req := KeyPairListRequest{}
			pg, e := NewKeyPairListPaginator(&cli, req, c.PGOptions)
			assert.Nil(t, e)
...
func TestKeyPairListPaginator_lastPage(t *testing.T) {
	...
	pg, e := NewKeyPairListPaginator(&cli, req, KeyPairPaginatorOptions{
		Limit:  3,
		Offset: 3,
	})
	assert.Nil(t, e)
