package storage

import (
	"context"
	"fmt"
	"github.com/clo-ru/cloapi-go-client/clo"
	"net/http"
)

const (
	s3UserListEndpoint = "/v1/projects/%s/s3_users"
)

type S3UserListRequest struct {
	clo.Request
	ProjectID string
}

func (r *S3UserListRequest) Make(ctx context.Context, cli *clo.ApiClient) (S3UserListResponse, error) {
	rawReq, e := r.buildRequest(ctx, cli.Options)
	if e != nil {
		return S3UserListResponse{}, e
	}
	rawResp, requestError := r.MakeRequest(rawReq, cli)
	if requestError != nil {
		return S3UserListResponse{}, requestError
	}
	defer rawResp.Body.Close()
	var resp S3UserListResponse
	if e = resp.FromJsonBody(rawResp.Body); e != nil {
		return S3UserListResponse{}, e
	}
	return resp, nil
}

func (r *S3UserListRequest) buildRequest(ctx context.Context, cliOptions map[string]interface{}) (*http.Request, error) {
	authKey, ok := cliOptions["auth_key"].(string)
	if !ok {
		return nil, fmt.Errorf("auth_key client options should be a string, %T got", authKey)
	}
	baseUrl, ok := cliOptions["base_url"].(string)
	if !ok {
		return nil, fmt.Errorf("base_url client options should be a string, %T got", baseUrl)
	}
	baseUrl += fmt.Sprintf(s3UserListEndpoint, r.ProjectID)
	rawReq, e := http.NewRequestWithContext(
		ctx, http.MethodGet, baseUrl, nil,
	)
	if e != nil {
		return nil, e
	}
	h := http.Header{}
	h.Add("Authorization", fmt.Sprintf("Bearer %s", authKey))
	r.WithHeaders(h)
	return rawReq, nil
}

func (r *S3UserListRequest) OrderBy(of string) *S3UserListRequest {
	r.WithQueryParams(map[string][]string{"order": {of}})
	return r
}

func (r *S3UserListRequest) FilterBy(ff FilteringField) *S3UserListRequest {
	switch ff.Condition {
	case "gt", "gte", "lt", "lte", "range", "in":
		condString := fmt.Sprintf("%s__%s", ff.FieldName, ff.Condition)
		r.WithQueryParams(map[string][]string{condString: {ff.Value}})
	}
	return r
}

type S3UserListPaginator struct {
	op       PaginatorOptions
	client   *clo.ApiClient
	params   S3UserListRequest
	lastPage bool
}

func (lp *S3UserListPaginator) LastPage() bool {
	return lp.lastPage
}

func NewS3UserListPaginator(client *clo.ApiClient, params S3UserListRequest, op PaginatorOptions) (*S3UserListPaginator, error) {
	if op.Limit == 0 {
		return nil, fmt.Errorf("op.Limit should not be 0")
	}
	lp := S3UserListPaginator{
		client: client,
		params: params,
		op:     op,
	}
	return &lp, nil
}

func (lp *S3UserListPaginator) NextPage(ctx context.Context) (S3UserListResponse, error) {
	if lp.LastPage() {
		return S3UserListResponse{}, fmt.Errorf("no more pages")
	}
	lp.params.WithQueryParams(map[string][]string{"limit": {fmt.Sprintf("%d", lp.op.Limit)}})
	lp.params.WithQueryParams(map[string][]string{"offset": {fmt.Sprintf("%d", lp.op.Offset)}})
	r, e := lp.params.Make(ctx, lp.client)
	if e != nil {
		return S3UserListResponse{}, e
	}
	lp.op.Offset += lp.op.Limit
	if r.Count <= lp.op.Limit+lp.op.Offset {
		lp.lastPage = true
	}
	return r, e
}
