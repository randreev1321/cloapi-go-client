...
func TestS3UserSuspendRequest_BuildRequest(t *testing.T) {
	...
	expReq, _ := http.NewRequestWithContext(
		context.Background(), http.MethodPost, mocks.MockUrl+fmt.Sprintf(s3UserSuspendEndpoint, ID), nil,
	)
	expReq.Header = h
...
func TestS3UserSuspendRequest_Make(t *testing.T) {
	...
	var cases = []struct {
		...
		Expected       S3UserSuspendResponse
	}{
		{
			Name: "Success",
			BodyStringFunc: func() (string, int) {
			...
			Req: S3UserSuspendRequest{},
			Expected: S3UserSuspendResponse{Result: ResponseItem{
				ID:         "user_id",
				MaxBuckets: 2,
				Quotas: []QuotaInfo{
					{
